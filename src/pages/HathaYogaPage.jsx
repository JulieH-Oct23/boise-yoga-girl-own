  ...
  useEffect(() => {
    async function fetchHathaSequences() {
      try {
        ...
        // Filter sequences with capitalized 'Hatha'
        const filtered = res.data.filter(seq => seq.style === "Hatha");
  ...
  return (
    <Box p={6}>
      ...
      <Text fontSize="lg" color={textColor} mb={6}>
        ...
        Hatha yoga is characterized by slower, deliberate movements, an emphasis on alignment and posture, longer holds in poses, and suitability for beginners and people of all ages. It can be both therapeutic and restorative, providing numerous benefits such as improving physical strength, balance, and flexibility, reducing stress and anxiety, enhancing focus and body awareness, and preparing the mind for meditation. Its gentle yet mindful approach makes it an excellent choice for anyone seeking to cultivate both physical and mental well-being.
      </Text>
      <Text fontSize="lg" color={textColor} mb={6}>
        Yin & Yang Poses can look very similar, but are practiced differently. A great example is the difference between Lunges(Yang) and Dragons(Yin). Lunges(Yang) are practiced actively, often with the back leg lifted and engaged. The spine is upright, the core is on, and breath fuels muscular strength. Inhale to lengthen the back leg and lift the chest; exhale to stabilize and deepen. In contrast, the Yin version, Dragon, begins similarly from Downward Dog, but once the knee is down and hands or forearms are grounded, there is no muscle activation—only surrender. The breath is slow and minimal effort is used as you allow connective tissue to stretch with time. There’s no pushing, no reaching—only staying. Yin is passive and meditative, Yang is dynamic and energizing.
      </Text>
