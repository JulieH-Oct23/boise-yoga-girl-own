...
const SequenceBuilderPage = () => {
  ...
  // Use Chakra color mode for bg and text colors on cards
    ...
    <Box p={4}>
              ...
              <HStack wrap="wrap" spacing={3}>
                {selectedPoses.map((pose, index) => (
                  <Box
                    key={`${pose._id}-${index}`}
                    p={2}
                    bg={cardBg} // use light/dark color
                    border="1px solid #ccc"
                    borderRadius="md"
                    w="100px"
                    textAlign="center"
                    position="relative"
                    color={cardTextColor} // text color too
                  >
                    <Image
                      src={getImageSrc(pose.image)}
                      alt={pose.name}
      ...
      >
        {filteredPoses.map((pose) => (
          <Box
            key={pose._id}
            bg={cardBg} // use light/dark color here too
            p={2}
            borderRadius="lg"
            boxShadow="md"
            textAlign="center"
